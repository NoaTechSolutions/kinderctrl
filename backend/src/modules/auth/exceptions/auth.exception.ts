...
export class AuthException extends HttpException {
  ...
  // also block the Authorization header refresh path on the client; we want
  // to look like a normal credentials failure with extra metadata.
  ...
  // Used for both "no such token", "already used", and "expired" — the
  // generic message avoids telling an attacker WHICH failure mode hit, so
  // probing for valid tokens reveals nothing useful.
